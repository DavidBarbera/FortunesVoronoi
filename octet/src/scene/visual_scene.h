...
namespace octet { namespace scene {
  ...
  public:
	...
	/// set up OpenGL state
	void begin_render_view(int x, int y, int vx, int vy, vec4_in clear_color = vec4(0.5f, 0.5f, 0.5f, 1.0f)) {
		/// set a viewport - includes whole window area
		glViewport(x, y, vx, vy);
	...
	/// render using default shaders.
	void render_view(float aspect_ratio, camera_instance *cam) {
		if (camera_instances.size() != 0) {
			render_impl(*object_shader, *skin_shader, *cam, aspect_ratio);
		}
	}
